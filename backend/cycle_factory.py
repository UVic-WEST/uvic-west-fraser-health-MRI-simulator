...
class CycleFactory:
    ...
    def get_cycle_by_id(self, cycle_id: int) -> CycleConfig:
        ...
        if not isinstance(cycle_id, int):
            raise ValueError(f"Invalid cycle id type: {type(cycle_id)}")
    ...
    def refresh(self):
        """Re cycles from JSON (call after saving a new cycle)."""
        self._load_cycles()
    ...
    def add_custom_cycle(self, cycle: CycleConfig):
        ...
        # prevent modification of preset cycles
        if cycle.cycle_id in self.PRESET_IDS:
            raise ValueError(f"Cannot overwrite preset cycle ID {cycle.cycle_id}")
