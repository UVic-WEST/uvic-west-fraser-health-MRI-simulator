...
class SoundGroupSummaryWidget(QWidget):
    def __init__(self, controller, parent=None):
        ...
        Args:
            controller: create-cycle controller used to read group/sound state
            parent: parent summary page widget
        """
        ...
        self.group_selector_button.clicked.connect(self._go_to_sound_mapping_page)
    ...
    def _go_to_sound_mapping_page(self):
        """Navigate back to sound mapping and preselect the current group."""
        current = self.parent()
        while current is not None:
            if hasattr(current, "_ensure_sound_group_mapping_page") and hasattr(current, "main_layout"):
                current._ensure_sound_group_mapping_page()
                if hasattr(current, "cc_sound_group_mapping_page") and current.cc_sound_group_mapping_page is not None:
                    mapping_page = current.cc_sound_group_mapping_page
                    mapping_page.refresh_groups_from_backend()
                    target_group_name = f"GROUP {self.current_group_index}"
                    if hasattr(mapping_page, "group_options") and target_group_name in mapping_page.group_options:
                        mapping_page.group_dropdown.setCurrentText(target_group_name)
                        if hasattr(mapping_page, "on_group_changed"):
                            mapping_page.on_group_changed(target_group_name)
                current.main_layout.setCurrentIndex(3)
                return
